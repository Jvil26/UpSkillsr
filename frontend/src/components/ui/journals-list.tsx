...
export default function JournalsList({ userSkillId }: JournalsListProps) {
  ...
  const { data: userSkill, isFetching: isFetchingUserSkill } = useFetchUserSkillById(userSkillId);
  const [page, setPage] = useState<number>(1);
  const { data: journals, isFetching: isFetchingJournals } = useFetchJournalsByUserSkillId(userSkillId, page);
  ...
  return (
    <div className="h-screen bg-muted pt-[calc(var(--nav-height))]">
      {isFetchingJournals || isFetchingUserSkill ? (
